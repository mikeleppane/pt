...
**pt** is a modern Python task runner built on `uv` for dependency isolation and environment management. It provides task orchestration, profiles, task inheritance, PEP 723 support, variable templating, and flexible command execution.
...
```bash
pt run --inline "pytest tests/" --env DEBUG=1
pt run --inline "python deploy.py" --env STAGE=prod --cwd workspace/
```
...
uv run pytest tests/ --cov=src/pt --cov-report=term-missing  # With coverage

# Running pt
uv run pt <command>        # Run CLI via uv
python -m pt <command>     # Run CLI directly
```
...
Four major features were added to enhance pt's capabilities:
